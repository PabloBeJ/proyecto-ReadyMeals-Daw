...
class UsuarioController extends Controller
{
    ...
    public function actualizar(UsuarioRequest $request, $id)
    {
        ...
        $this->validate($request, [
            'usuario' => 'required|string|max:255|unique:usuarios,usuario,' . $row->id,
            'nombre' => 'required|string|max:255',
            'email' => 'required|email|unique:usuarios,email,' . $row->id, // Ensure email is unique except for the current user
        ]);
    ...
    public function borrar($id)
    {
        ...
        if (file_exists($imagen) && basename($imagen) !== 'chef.png') {
            unlink($imagen);
        }
